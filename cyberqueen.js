...
const {Message, StringSession, Image, Video} = require('./CyberQueen/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ v3.9.9 - CMD BOT Public Edition 💃♥️')
        );
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/CyberQueen-alive.png"), MessageType.image, { caption: '*᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ ᴄᴍᴅ ʙᴏᴛ private ආකාරයට ක්‍රියා කරයි.*\n\n*Username:* ' + conn.user.name + '\n\n_මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකය වේ._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command බාවිතා කල හැකිය.*\n\n*ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.var WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ භාවිතා කිරීම ගැන ස්තූතියි.*', });
                }
            }
            else {
    ...
    conn.on('message-new', async msg => {
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                await conn.sendMessage(msg.key.remoteJid, fs.readFileSync("/root/CyberQueen/media/gif/VID-20210616-WA1216.mp4"), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message});
            }
            return;
            ...
            if (gb !== false) {
                await conn.sendMessage(msg.key.remoteJid, fs.readFileSync("/root/CyberQueen/media/gif/VID-20210616-WA0310.mp4"), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message});
            }
            return;
        }
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                   ...
                   /*if ((OWN.ff == "94752279625,94761209144,94770029091,94761219834,94717242063" && msg.key.fromMe === false && command.fromMe === true &&
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*-- දෝෂ වාර්තාව [᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ] --*' + 
